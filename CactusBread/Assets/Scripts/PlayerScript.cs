...
public class PlayerScript : MonoBehaviour {
    ...
    [SerializeField]
    GameObject bulletPrefab, casingPrefab, rokkitPrefab;
    bool forward;
    public enum gunType {revolver, rifle, rokkit };
    ...
    float ammoRokkitMax = 10, ammoRokkitCur = 10, ammoInRokkit = 1;
    [SerializeField]
    Text RifleAmmoCounter, RevolverAmmoCounter, RokkitAmmoCounter;
    ...
    void ShowAmmo()
    {
        ...
        string rokkit = "x/x";
        rokkit = ammoInRokkit + "/" + ammoRokkitCur;
        RokkitAmmoCounter.text = rokkit;
    }
    void GetPlayerInput()
    {
    ...
    void ReloadGun()
    {
        ...
        else if(equippedGun == gunType.rifle)
        {
        ...
        else if (equippedGun == gunType.rokkit)
        {
            if (ammoRokkitCur - 1 >= 0)
            {
                ammoRokkitCur -= 1 - ammoInRokkit;
                ammoInRokkit = 1;
            }
            else
            {
                ammoInRokkit = ammoRokkitCur;
                ammoRokkitCur = 0;
            }
            for (int x = 0; x <= 0; x++)
            {
    ...
    void Fire()
    {
        ...
        if (equippedGun == gunType.revolver)
            ...
            go.GetComponent<Rigidbody2D>().AddForce((position - go.transform.position).normalized * 700);
    }
    ...
    void FireRokkit()
    {
        Vector3 position = new Vector3(Input.mousePosition.x, Input.mousePosition.y, 0);
        position = Camera.main.ScreenToWorldPoint(position);
        GameObject go = Instantiate(rokkitPrefab, transform.position, Quaternion.identity) as GameObject;
        go.transform.LookAt(position);
